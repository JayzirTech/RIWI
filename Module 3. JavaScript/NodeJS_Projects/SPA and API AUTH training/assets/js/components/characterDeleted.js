...
export function characterDeletedCard(character) {
    ...
    return `
        <article id="${character.id}" class="overflow-hidden rounded-[28px] border border-slate-800 bg-slate-950/95 shadow-[0_24px_80px_rgba(14,19,32,0.7)] transition-transform duration-200 hover:-translate-y-1 hover:shadow-cyan-500/15">
            ...
            <div class="p-5 space-y-3">
                ...
                <button id="restore-${character.id}" class="relative top-4 left-5 rounded-full bg-cyan-500/30 px-4 py-2 text-sm font-semibold text-cyan-300 hover:bg-cyan-500">Restore</button>
            </div>
        </article>
    `;
}
