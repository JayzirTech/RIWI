    ...
    if (confirm('Are you sure you want to restore this character?')) {
        ...
        if (article) {
            ...
            setTimeout(() => {
                ...
                apiData.unshift(charactersDeleted.find(char => char.id === parseInt(id))); // Agrego el personaje eliminado al array
                charactersDeleted = charactersDeleted.filter(char => char.id !== parseInt(id)); // Quito el personaje eliminado del array
